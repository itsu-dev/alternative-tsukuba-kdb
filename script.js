...
window.onload = function () {
  ...
  clearButton.addEventListener(
    ...
    (clearButtonListener = (evt) => {
      ...
      for (let x = 0; x < 7; x++) for (let y = 0; y < 6; y++) selectedPeriods[x][y] = false;
            ...
            for (let yi = 0; yi < maxPeriod; yi++) {
              for (let xi = 0; xi < daysofweek.length; xi++) {
                ...
                if (
                  ...
                  if (selectedDownPeriods[xi][yi]) target.classList.remove('selected');
                  else target.classList.add('selected');
                  selectedPeriods[xi][yi] = !selectedDownPeriods[xi][yi];
                } else {
                  if (selectedDownPeriods[xi][yi]) target.classList.add('selected');
  ...
  const createLine = (line) => {
    ...
    let url = `https://kdb.tsukuba.ac.jp/syllabi/2022/${line.code}/jpn`;
  ...
  const updateTable = (options, index, displayedIndex) => {
    ...
    for (;;) {
      ...
      if (line.year.indexOf('-') < 0)
        ...
        missMatchesYear =
          options.year != 'null' && (options.year < minYear || maxYear < options.year);
      }
  ...
  const search = (e) => {
    ...
    options.reqB =
      reqB_input.selectedIndex > -1 ? reqB_input.options[reqB_input.selectedIndex].value : 'null';
    options.reqC =
      reqC_input.selectedIndex > -1 ? reqC_input.options[reqC_input.selectedIndex].value : 'null';
        ...
        for (let period of periodStrArray) {
          ...
          if (timeStr.length > 0)
            for (let day of dayArray)
              for (let time of timeArray) line.period[j].period[day][time - 1] = true;
        }
      }
...
class BookmarkTimetable {
  ...
  shiftTimetable(isForward) {
    const maxModule = 5;
    if (isForward && this.module < maxModule) this.switchTimetable(this.module + 1);
